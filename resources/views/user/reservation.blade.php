                            ...
                            @foreach($reservations as $key=>$allReservations)
                                <tr>
                                    ...
                                    <td>
                                        ...
                                        {{-- <form id="status-form-{{ $allReservations->id }}" action="{{ route('showReservations',$allReservations->id) }}" 
                                        ...
                                        <button type="button" class="btn btn-info btn-sm" onclick="if(confirm('هل تريد تأكيد الطلب والموافقة عليه ؟')){
                                                ...
                                                event.preventDefault();
                                                 ...
                                                 --}}
                                    {{-- if($allReservations->reservation == false && $allReservations->state == "بيع") --}}
                                    <form id="delete-form-{{ $allReservations->id }}" action="{{ route('reservationsUser.destroy',$allReservations->id) }}" 
                                    style="display: none;" method="POST">
                                        @csrf
                                        @method('DELETE')
                                    </form>

                                    <button type="button" class="btn btn-danger btn-sm" onclick="if(confirm('هل أنت متأكد أنك تريد إلغاء الحجز ؟')){
                                            event.preventDefault();
                                             document.getElementById('delete-form-{{ $allReservations->id }}').submit();
                                                }else {
                                                event.preventDefault();
